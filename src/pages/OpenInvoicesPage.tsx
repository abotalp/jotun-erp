...
import {
  ...
  ShoppingCart, Receipt, Printer
} from 'lucide-react'

// ═══════════════════════════════════════════════════════
// الصفحة الرئيسية للفواتير المفتوحة
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// نافذة الفاتورة الكاملة (تشبه الكاشير)
// ═══════════════════════════════════════════════════════
function FullInvoiceModal({ saleId, onClose, onChange }: any) {
  ...
  useEffect(() => {
    db.product_variants()
      .select(`id, size_name, sku, barcode, cost_price, sale_price, image_url, product:products!inner(id, name, is_active, category_id, main_image_url)`)
      .eq('is_active', true)
      .eq('product.is_active', true)
      .order('sku')
      .limit(2000)
      .then(({ data }) => {
        if (data) setAllVariants(data)
      })
  ...
  const filteredVariants = useMemo(() => {
    ...
    if (activeCategory) {
      result = result.filter((v: any) => v.product?.category_id === activeCategory)
    }
    if (search.length >= 1) {
  ...
  async function load() {
    ...
    if (r.success) {
      ...
      setDiscountValue(r.invoice.discount_value ?? 0)
      setDiscountType((r.invoice.discount_type as any) ?? 'percentage')
    }
    setLoading(false)
  }

  // إضافة منتج للفاتورة (بضغطة واحدة)
  async function handleQuickAdd(v: any) {
    if (!activeWarehouse?.id) { alert('لا يوجد مخزن'); return }
    const r = await addItemToOpenInvoice(saleId, {
      variantId: v.id,
  ...
  async function handleUpdateItemQty(itemId: string, newQty: number) {
    ...
    // إضافة بالكمية الجديدة
    await addItemToOpenInvoice(saleId, {
      ...
      color: item.color?.[0] ? {
        colorId: item.color[0].color_id,
        colorCode: item.color[0].color_code,
        colorName: item.color[0].color_name
      } : undefined
    }, activeWarehouse.id, user?.id)
  ...
  async function handlePay() {
    ...
    if (r.success) {
      setPayAmount(0); setPayNotes(''); setShowPaymentForm(false)
      load(); onChange?.()
    } else alert(r.error)
  }

  async function handleDeletePayment(paymentId: string, amount: number) {
    if (!confirm(`حذف دفعة بقيمة ${formatCurrency(amount)}؟`)) return
  ...
  async function handleCloseAndPrint() {
    if ((invoice.items?.length ?? 0) === 0) { alert('لا يمكن إغلاق فاتورة فارغة'); return }
    if (!confirm('إغلاق الفاتورة نهائياً والطباعة؟')) return
    ...
    setPrintData({
      ...
      total,
      paid: invoice.paid,
      remaining: Math.max(0, total - invoice.paid),
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[95vh] flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-gray-50">
            <div className="p-3 border-b bg-white">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pr-9 pl-3 py-2.5 border-2 rounded-xl text-sm outline-none focus:border-amber-400 bg-gray-50"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="px-3 py-2 border-b bg-white flex gap-1.5 overflow-x-auto">
              <button onClick={() => setActiveCategory(null)} className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap ${activeCategory === null ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                الكل ({allVariants.length})
              </button>
              {categories.map(cat => (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {filteredVariants.map((v: any) => (
                  <button
                    ...
                    className="bg-white rounded-xl border-2 border-gray-100 p-3 text-right hover:border-amber-400 hover:shadow-md transition-all active:scale-95"
                  >
                    <div className="w-full h-16 rounded-lg mb-2 flex items-center justify-center text-2xl" style={{ background: 'linear-gradient(135deg, #1B2E4B, #1B3A6B)' }}>
                      🎨
                    </div>
          ...
          <div className="w-[400px] flex-shrink-0 bg-white flex flex-col border-r-2 border-gray-100">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {item.color?.[0]?.color_code && (
                        <p className="text-[10px] text-purple-600 mt-0.5">🎨 {item.color[0].color_code} - {item.color[0].color_name}</p>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      ...
                      <input
                        type="number"
                        value={item.quantity}
                        onChange={e => handleUpdateItemQty(item.id, +e.target.value || 1)}
                        min={1}
                        className="w-full px-2 py-1 border rounded-lg text-center font-bold"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => handleUpdateItemPrice(item.id, +e.target.value || 0)}
                        step="0.01"
                        className="w-full px-2 py-1 border rounded-lg text-center font-bold"
                      />
                    </div>
            ...
            <div className="border-t bg-white p-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={discountType} onChange={e => setDiscountType(e.target.value as any)} className="text-xs border rounded-lg px-2 py-1 outline-none">
                ...
                <input
                  ...
                  placeholder="0"
                  className="flex-1 text-xs border rounded-lg px-2 py-1 outline-none text-center"
                  min={0}
                />
              </div>
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between font-black text-base pt-1 border-t">
                  <span>الإجمالي</span>
                  <span style={{ color: '#D4AF37' }}>{formatCurrency(finalTotal)}</span>
                </div>
                <div className="flex justify-between text-green-600 text-xs">
                  <span>المدفوع</span>
                  <span>{formatCurrency(invoice.paid)}</span>
                </div>
                {(finalTotal - invoice.paid) > 0 && (
                  <div className="flex justify-between text-red-600 font-bold text-sm">
                    <span>المتبقي</span>
                    <span>{formatCurrency(finalTotal - invoice.paid)}</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setShowPaymentForm(true)} className="py-2 bg-green-600 text-white rounded-xl text-xs font-bold hover:opacity-90">
                  💰 إضافة دفعة
                </button>
                <button onClick={() => setShowPaymentsList(true)} className="py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:opacity-90">
              ...
              <button
                ...
                disabled={(invoice.items?.length ?? 0) === 0}
                className="w-full py-3 text-white font-black rounded-xl shadow-lg hover:opacity-90 active:scale-95 disabled:opacity-50"
      ...
      {showColorPicker && (
        <ColorPicker
          onSelect={(c) => handleAddColor(showColorPicker, c)}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={() => setShowPaymentForm(false)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex justify-between mb-4">
              <h3 className="text-lg font-black text-green-600">💰 دفعة جديدة</h3>
              <button onClick={() => setShowPaymentForm(false)}><X size={20} /></button>
            </div>
            <div className="bg-red-50 rounded-xl p-3 mb-4 text-center">
              <p className="text-xs text-gray-500">المتبقي</p>
              <p className="text-2xl font-black text-red-600">{formatCurrency(finalTotal - invoice.paid)}</p>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-bold mb-1">المبلغ</label>
                <input type="number" value={payAmount || ''} onChange={e => setPayAmount(+e.target.value || 0)} className="w-full px-3 py-3 border-2 rounded-xl text-2xl font-black text-center focus:border-green-400 outline-none" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setPayAmount(finalTotal - invoice.paid)} className="py-2 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold">دفع الكل</button>
              ...
              <div>
                ...
                <select value={payMethod} onChange={e => setPayMethod(e.target.value)} className="w-full px-3 py-2.5 border-2 rounded-xl bg-white">
                  <option value="cash">نقدي</option>
                  <option value="bank_transfer">تحويل</option>
                  <option value="visa">فيزا</option>
                </select>
              </div>
              <input value={payNotes} onChange={e => setPayNotes(e.target.value)} placeholder="ملاحظات..." className="w-full px-3 py-2.5 border-2 rounded-xl text-sm" />
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={() => setShowPaymentsList(false)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col">
            <div className="flex justify-between p-5 border-b">
              <h3 className="text-lg font-black text-blue-600">📜 الدفعات</h3>
              <button onClick={() => setShowPaymentsList(false)}><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {(invoice.payments ?? []).length === 0 ? (
                <div className="text-center py-12 text-gray-400">لا توجد دفعات</div>
                ...
                <div key={p.id} className="bg-gray-50 rounded-xl p-3 flex justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setEditingPayment(p)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg">
                      <Edit2 size={14} />
                    </button>
                    <button onClick={() => handleDeletePayment(p.id, p.amount)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex justify-between p-5 border-b">
          <h3 className="text-lg font-black text-blue-600">✏️ تعديل دفعة</h3>
...
function NewInvoiceModal({ onClose, onCreated }: any) {
  ...
  async function handleCreate(customerId: string) {
    ...
    setCreating(false)
    if (r.success) { onCreated?.(r.sale.id); onClose() }
    else alert(r.error)
  }
  ...
  async function handleAddNew() {
    ...
    const { data: newCust, error } = await db.customers().insert({
      name: newName.trim(), phone: newPhone.trim() || null, code, customer_type: newType, is_active: true
    }).select().single()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] flex flex-col">
        <div className="flex justify-between p-4 border-b">
          <h3 className="text-lg font-black">{showNewCustomer ? '➕ عميل جديد + فاتورة' : '➕ فاتورة جديدة'}</h3>
          <button onClick={onClose}><X size={20} /></button>
        </div>
        {showNewCustomer ? (
          <>
            ...
            <div className="p-4 border-t flex gap-2">
              ...
              <button onClick={handleAddNew} disabled={creating} className="flex-1 py-2.5 text-white font-bold rounded-xl" style={{ background: 'linear-gradient(to left, #F59E0B, #D97706)' }}>
                {creating ? '⏳' : '✅ إنشاء'}
              </button>
            </div>
          </>
          ...
          <>
            <div className="p-3 border-b space-y-2">
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 ابحث..." className="w-full px-3 py-2 border-2 rounded-lg" />
              <button onClick={() => setShowNewCustomer(true)} className="w-full py-2 bg-amber-50 text-amber-700 rounded-lg font-bold flex justify-center items-center gap-2">
