...
export const useAppStore = create<AppStore>()(
  persist(
    (set, get) => ({
      ...
      addToCart: (item) => set(s => ({ cart: [...s.cart, calcRow(item)] })),
      removeFromCart: (rowId) => set(s => ({ cart: s.cart.filter(i => i.rowId !== rowId) })),
      updateCartItem: (rowId, updates) => set(s => ({
        cart: s.cart.map(i => i.rowId === rowId ? calcRow({ ...i, ...updates }) : i)
      })),
      clearCart: () => set({
