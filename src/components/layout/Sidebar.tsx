...
const NAV_ITEMS = [
  ...
  { key: 'open-invoices', icon: FolderOpen,      label: 'فواتير مفتوحة',     roles: ['admin','manager','cashier'] },
  { key: 'payments',      icon: CreditCard,      label: 'المدفوعات والسندات', roles: ['admin','manager','cashier'] },
  ...
  { key: 'customers',     icon: Users,           label: 'العملاء والموزعين', roles: ['admin','manager','cashier'] },
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 p-4 border-b border-white/10 min-h-[64px]">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-lg" style={{ background: 'linear-gradient(135deg, #D4AF37, #B8960C)' }}>
          🎨
        </div>
        {sidebarOpen && (
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1 rounded-lg hover:bg-white/10 transition-colors">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all w-full ${
                isActive ? 'text-white shadow-lg' : 'text-blue-200 hover:bg-white/10 hover:text-white'
              }`}
              style={isActive ? {
      ...
      <div className="border-t border-white/10 p-3 space-y-2">
        {sidebarOpen && user && (
          <div className="flex items-center gap-2 px-2 py-1">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#D4AF37' }}>
        ...
        <button onClick={signOut} className="flex items-center gap-3 w-full px-3 py-2 rounded-xl text-sm text-red-300 hover:bg-red-500/20 hover:text-red-200 transition-all">
